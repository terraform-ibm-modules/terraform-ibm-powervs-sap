...
| <a name="input_powervs_os_registration"></a> [powervs\_os\_registration](#input\_powervs\_os\_registration) | Specify whether IBM provided or customer provided linux subscription should be used. For IBM provided subscription set fls=true. For customer provided subscription set a username and a password inside byol. Only one can be used, fls=true + defining username and password will throw an error. Default: fls=true | <pre>object({<br/>    byol = object({<br/>      username = string<br/>      password = string<br/>    })<br/>    fls = bool<br/>  })</pre> | <pre>{<br/>  "byol": {<br/>    "password": "",<br/>    "username": ""<br/>  },<br/>  "fls": true<br/>}</pre> | no |
