...
variable "ansible_vault_password" {
  description = "Vault password to encrypt ansible variable file for SAP installation. Password requirements: 15-100 characters and at least one uppercase letter, one lowercase letter, one number, one of the following special characters: \"!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~\". Avoid non-printable characters."
  type        = string
  sensitive   = true
  default     = null
  validation {
